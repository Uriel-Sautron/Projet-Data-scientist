...
def shortest_handle(journalists) #2
...
def handle_five_char(journalists) #3
    puts "Combien y-a-t'il de handle contenant 5 caractères" 
    print "> "
    puts journalists.count { |journalists| journalists == 6}
end 

handle_five_char(journalists)

def uppercase_handle(journalists) #4
  puts "Combien commencent par une majuscule ?" 
  count = 0

  journalists.each do |users|
  users_first_char = users[1]
      if users_first_char.match (/[A-Z]/)
        count +=1
      end
    end
    puts "il y a #{count} journalistes qui ont un pseudo commençant par une majuscule"
end

print uppercase_handle(journalists)

#  puts journalists

def sort_list(journalists) #5
  puts"Voici la liste trié par ordre alphabétique"
  puts "-" * 20
  puts journalists.sort
  puts "-" * 20
end

sort_list(journalists)

def index_epenser(journalists) #6
  puts "l'index de e-penser est le suivant"
  print ">"
  puts journalists.index ("@epenser")
  puts "-" * 20
end

index_epenser(journalists)


def sort_order(journalists)
